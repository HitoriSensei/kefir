...
function combineAsArray(active, passive = [], combinator, constructor) {
  ...
  combinator = combinator
    ? spread(combinator, active.length + passive.length)
    : x => x;
  return active.length === 0
    ? never()
    : new constructor(active, passive, combinator);
}
...
function combineAsObject(active, passive = {}, combinator, constructor) {
  ...
  return activeObservables.length === 0
    ? never()
    : new constructor(activeObservables, passiveObservables, objectify);
}

export function handleCombineParameters(
  active,
  passive,
  combinator,
  constructor
