...
describe("takeUntilBy", () => {
  ...
  describe("stream, stream", () => {
    ...
    it("should be ended if primary was ended", () =>
      expect(send(stream(), [end()]).takeUntilBy(stream())).to.emit([
        end({ current: true })
      ]));
    ...
    it("should emit values from primary until first value from secondary", () => {
      const a = stream();
      const b = stream();
      expect(a.takeUntilBy(b)).to.emit([value(1), value(2)], () =>
  ...
  describe("stream, property", () => {
    ...
    it("should be ended if primary was ended", () =>
      expect(send(stream(), [end()]).takeUntilBy(prop())).to.emit([
        end({ current: true })
      ]));
    ...
    it("should be ended if secondary was ended and has any current", () => {
      expect(stream().takeUntilBy(send(prop(), [value(0), end()]))).to.emit([
        end({ current: true })
      ]);
    });
  ...
  describe("property, stream", () => {
    ...
    it("should be ended if primary was ended", () =>
      expect(send(prop(), [end()]).takeUntilBy(stream())).to.emit([
        end({ current: true })
      ]));
    ...
    it("should take values as expected", () => {
      ...
      const b = stream();
      expect(a.takeUntilBy(b)).to.emit(
        [value(0, { current: true }), value(3), value(4), end()],
        () => {
          send(a, [value(3), value(4)]);
          send(b, [value(1)]);
          send(a, [value(5), value(6)]);
  ...
  describe("property, property", () => {
    it("should return a property", () =>
      expect(prop().takeUntilBy(prop())).to.be.observable.property());
    ...
    it("should be ended if primary was ended", () =>
      expect(send(prop(), [end()]).takeUntilBy(prop())).to.emit([
        end({ current: true })
      ]));
    ...
    it("should be ended if secondary was ended and has any current", () =>
      expect(prop().takeUntilBy(send(prop(), [value(0), end()]))).to.emit([
        end({ current: true })
      ]));
