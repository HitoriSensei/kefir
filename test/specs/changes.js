...
describe("changes", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(stream(), [end()]).changes()).to.emit([
        end({ current: true })
      ]);
    });

    it("test `streamWithCurrent` helper", () => {
      expect(streamWithCurrent({ type: "value", value: 1 })).to.emit([
        value(1, { current: true })
      ]);
      expect(streamWithCurrent({ type: "error", value: 1 })).to.emit([
    ...
    it("should handle events and current", () => {
      ...
      expect(a.changes()).to.emit(
        [value(value(2)), error(5), value(value(3)), end()],
