...
describe("map", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).map(() => {})).to.emit([
        end({ current: true })
      ]));

    it("should handle events", () => {
      const a = stream();
      expect(a.map(x => x * 2)).to.emit(
        [value(2), error(5), value(4), end()],
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).map(() => {})).to.emit([
        end({ current: true })
      ]));
    ...
    it("should handle events and current", () => {
      ...
      expect(a.map(x => x * 2)).to.emit(
        [value(2, { current: true }), value(4), error(5), value(6), end()],
        () => send(a, [value(2), error(5), value(3), end()])
      );
      a = send(prop(), [error(0)]);
      expect(a.map(x => x * 2)).to.emit(
        [error(0, { current: true }), value(4), error(5), value(6), end()],
        () => send(a, [value(2), error(5), value(3), end()])
      );
    });
  });
