...
describe("Property", () => {
  ...
  describe("end", () => {
    ...
    it("should stop deliver new values after end", () => {
      const s = prop();
      expect(s).to.emit([value(1), value(2), end()], () =>
        send(s, [value(1), value(2), end(), value(3)])
      );
    });
  ...
  describe("subscribers", () => {
    it("should deliver values and current", () => {
      ...
      expect(s).to.emit([value(0, { current: true }), value(1), value(2)], () =>
        send(s, [value(1), value(2)])
      );
    });

    it("should deliver errors and current error", () => {
      const s = send(prop(), [error(0)]);
      expect(s).to.emit([error(0, { current: true }), error(1), error(2)], () =>
        send(s, [error(1), error(2)])
      );
    });
