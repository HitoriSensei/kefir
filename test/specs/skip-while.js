...
describe("skipWhile", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).skipWhile(() => false)).to.emit([
        end({ current: true })
      ]));

    it("should handle events (`-> true`)", () => {
      const a = stream();
      expect(a.skipWhile(() => true)).to.emit([end()], () =>
    ...
    it("should handle events (`-> false`)", () => {
      const a = stream();
      expect(a.skipWhile(() => false)).to.emit(
    ...
    it("should handle events (`(x) -> x < 3`)", () => {
      const a = stream();
      expect(a.skipWhile(x => x < 3)).to.emit(
        [value(3), value(4), value(5), end()],
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).skipWhile(() => false)).to.emit([
        end({ current: true })
      ]));
