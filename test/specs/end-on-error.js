...
describe("endOnError", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(stream(), [end()]).endOnError()).to.emit([
        end({ current: true })
      ]);
    });
    ...
    it("should handle events", () => {
      let a = stream();
      expect(a.endOnError()).to.emit([value(1), error(5), end()], () =>
        send(a, [value(1), error(5), value(2)])
      );
      a = stream();
      expect(a.endOnError()).to.emit([value(1), value(2), end()], () =>
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(prop(), [end()]).endOnError()).to.emit([
        end({ current: true })
      ]);
    });
    ...
    it("should handle events and current", () => {
      ...
      expect(a.endOnError()).to.emit(
        [value(1, { current: true }), error(5), end()],
        () => send(a, [error(5), value(2)])
      );
      a = send(prop(), [value(1)]);
      expect(a.endOnError()).to.emit(
