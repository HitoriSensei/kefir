...
describe("bufferBy", () => {
  describe("common", () => {
    ...
    it("should end when primary ends", () => {
      expect(send(stream(), [end()]).bufferBy(stream())).to.emit([
        value([], { current: true }),
        end({ current: true })
      ]);
      const a = stream();
    ...
    it("should flush buffer on end", () => {
      ...
      const a = stream();
      const b = stream();
      expect(a.bufferBy(b)).to.emit([value([1, 2]), end()], () =>
    ...
    it("should not flush buffer on end if {flushOnEnd: false}", () => {
      expect(
        send(prop(), [value(1), end()]).bufferBy(stream(), {
          flushOnEnd: false
        })
      ).to.emit([end({ current: true })]);
      const a = stream();
      const b = stream();
      expect(a.bufferBy(b, { flushOnEnd: false })).to.emit([end()], () =>
        send(a, [value(1), value(2), end()])
      );
    });
    ...
    it("should do end when secondary ends if {flushOnEnd: false}", () => {
      expect(
        stream().bufferBy(send(stream(), [end()]), { flushOnEnd: false })
      ).to.emit([end({ current: true })]);
      const a = stream();
      const b = stream();
      expect(a.bufferBy(b, { flushOnEnd: false })).to.emit([end()], () =>
    ...
    it("should flush buffer on each value from secondary", () => {
      const a = stream();
      const b = stream();
      expect(a.bufferBy(b)).to.emit(
        [value([]), value([1, 2]), value([]), value([3])],
        () => {
          send(b, [value(0)]);
