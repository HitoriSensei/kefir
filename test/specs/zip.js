...
describe("zip", () => {
  ...
  it("should be ended and has current if array of ended properties provided and each of them has current", () => {
    ...
    expect(Kefir.zip([a, b, c])).to.emit([
      value([1, 2, 3], { current: true }),
      end({ current: true })
    ]);
    expect(a.zip(b)).to.emit([
  ...
  it("should handle events and current from observables", () => {
    ...
    //   [1,0,3] [4,2,5]   [6,9,8]
    expect(Kefir.zip([a, b, c])).to.emit(
      [value([1, 0, 3]), value([4, 2, 5]), value([6, 9, 8]), end()],
      () => {
        send(a, [value(1)]);
        send(b, [value(2)]);
        send(c, [value(3)]);
  ...
  it("should support arrays", () => {
    ...
    a = [1, 3];
    b = send(prop(), [value(0)]);
    expect(b.zip(a)).to.emit(
      [value([0, 1], { current: true }), value([2, 3]), end()],
      () => {
        send(b, [value(2)]);
        send(b, [end()]);
      }
  ...
  it("should accept optional combinator function", () => {
    ...
    let b = send(prop(), [value(0)]);
    const c = stream();
    expect(Kefir.zip([a, b, c], join)).to.emit(
      [value("1+0+3"), value("4+2+5"), value("6+9+8"), end()],
      () => {
        send(a, [value(1)]);
        send(b, [value(2)]);
