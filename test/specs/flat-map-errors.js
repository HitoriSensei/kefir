...
describe("flatMapErrors", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).flatMapErrors()).to.emit([
        end({ current: true })
      ]));
    ...
    it("should handle events", () => {
      ...
      expect(a.flatMapErrors()).to.emit(
        [value(1), value(2), value(0), value(3), value(4), end()],
        () => {
          send(b, [value(0)]);
          send(a, [error(b)]);
    ...
    it("should accept optional map fn", () => {
      const a = stream();
      const b = stream();
      expect(a.flatMapErrors(x => x.obs)).to.emit(
        [value(1), value(2), end()],
        () => {
          send(b, [value(0)]);
    ...
    it("should correctly handle current values of new sub sources", () => {
      ...
      const b = send(prop(), [value(1)]);
      const c = send(prop(), [value(2)]);
      expect(a.flatMapErrors()).to.emit([value(1), value(2)], () =>
        send(a, [error(b), error(c)])
      );
    });
    ...
    it("values should flow", () => {
      const a = stream();
      expect(a.flatMapErrors()).to.emit([value(1), value(2), value(3)], () =>
  ...
  describe("property", () => {
    ...
    it("values should flow", () => {
      const a = send(prop(), [value(0)]);
      expect(a.flatMapErrors()).to.emit(
        [value(0, { current: true }), value(1), value(2), value(3)],
        () => send(a, [value(1), value(2), value(3)])
      );
    });
  });
