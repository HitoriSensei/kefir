...
describe("valuesToErrors", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).valuesToErrors(() => {})).to.emit([
        end({ current: true })
      ]));
    ...
    it("should handle events", () => {
      ...
      expect(a.valuesToErrors(handler)).to.emit(
        [value(1), error(-6), error(-3), error(-12), value(5), end()],
        () =>
    ...
    it("default handler should convert all values", () => {
      const a = stream();
      expect(a.valuesToErrors()).to.emit(
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).valuesToErrors(() => {})).to.emit([
        end({ current: true })
      ]));
