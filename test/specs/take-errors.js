...
describe("takeErrors", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).takeErrors(3)).to.emit([
        end({ current: true })
      ]));
    ...
    it("should handle events (more than `n`)", () => {
      const a = stream();
      expect(a.takeErrors(3)).to.emit(
        [error(1), error(2), error(3), end()],
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).takeErrors(3)).to.emit([
        end({ current: true })
      ]));

    it("should be ended if `n` is 0", () =>
      expect(prop().takeErrors(0)).to.emit([end({ current: true })]));

    it("should handle events and current (less than `n`)", () => {
      const a = send(prop(), [error(1)]);
      expect(a.takeErrors(3)).to.emit(
        [error(1, { current: true }), error(2), end()],
        () => send(a, [error(2), end()])
      );
    });
    ...
    it("should handle events and current (more than `n`)", () => {
      const a = send(prop(), [error(1)]);
      expect(a.takeErrors(3)).to.emit(
        [error(1, { current: true }), error(2), error(3), end()],
        () => send(a, [error(2), error(3), error(4), error(5), end()])
      );
    });

    it("should work correctly with .constant", () =>
      expect(Kefir.constantError(1).takeErrors(1)).to.emit([
    ...
    it("values should flow", () => {
      ...
      expect(a.takeErrors(1)).to.emit(
        [value(1, { current: true }), value(2), value(3), value(4), end()],
