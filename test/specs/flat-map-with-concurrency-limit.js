...
describe("flatMapConcurLimit", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).flatMapConcurLimit(null, 1)).to.emit([
        end({ current: true })
      ]));
    ...
    it("should handle events", () => {
      ...
      expect(a.flatMapConcurLimit(null, 2)).to.emit(
        [value(1), value(2), value(4), value(5), value(6), end()],
        () => {
          send(b, [value(0)]);
          send(a, [value(b)]);
          send(b, [value(1)]);
    ...
    it("should accept optional map fn", () => {
      const a = stream();
      const b = stream();
      expect(a.flatMapConcurLimit(x => x.obs, 1)).to.emit(
        [value(1), value(2), end()],
        () => {
          send(b, [value(0)]);
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).flatMapConcurLimit(null, 1)).to.emit([
        end({ current: true })
      ]));
    ...
    it("should correctly handle current value of source", () => {
      ...
      expect(b.flatMapConcurLimit(null, 1)).to.emit([
