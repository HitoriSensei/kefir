...
describe("sampledBy", () => {
  ...
  it("should handle events and current from observables", () => {
    ...
    expect(a.sampledBy(b)).to.emit(
      [value(2), value(4), value(4), end()],
      () => {
        send(b, [value(1)]);
        send(a, [value(2)]);
        send(b, [value(3)]);
  ...
  it("should accept optional combinator function", () => {
    ...
    const a = stream();
    const b = send(prop(), [value(0)]);
    expect(a.sampledBy(b, join)).to.emit(
      [value("2+3"), value("4+5"), value("4+6"), end()],
      () => {
        send(b, [value(1)]);
        send(a, [value(2)]);
        send(b, [value(3)]);
  ...
  it("should work nice for emitating atomic updates", () => {
    ...
    const b = a.map(x => x + 2);
    const c = a.map(x => x * 2);
    expect(b.sampledBy(c, (x, y) => [x, y])).to.emit(
