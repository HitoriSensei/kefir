...
describe("errorsToValues", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(stream(), [end()]).errorsToValues(() => {})).to.emit([
        end({ current: true })
      ]);
    });
    ...
    it("should handle events", () => {
      ...
      expect(a.errorsToValues(handler)).to.emit(
        [value(1), value(6), error(-1), value(9), value(4), end()],
        () =>
    ...
    it("default handler should convert all errors", () => {
      const a = stream();
      expect(a.errorsToValues()).to.emit(
        [value(1), value(2), value(-1), value(3), value(4), end()],
        () =>
          send(a, [value(1), error(2), error(-1), error(3), value(4), end()])
      );
    });
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(prop(), [end()]).errorsToValues(() => {})).to.emit([
        end({ current: true })
      ]);
    });
