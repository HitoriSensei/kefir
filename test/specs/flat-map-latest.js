...
describe("flatMapLatest", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).flatMapLatest()).to.emit([
        end({ current: true })
      ]));
    ...
    it("should handle events", () => {
      ...
      expect(a.flatMapLatest()).to.emit(
        [value(1), value(0), value(3), value(5), end()],
        () => {
          send(b, [value(0)]);
          send(a, [value(b)]);
          send(b, [value(1)]);
    ...
    it("should accept optional map fn", () => {
      const a = stream();
      const b = stream();
      expect(a.flatMapLatest(x => x.obs)).to.emit(
        [value(1), value(2), end()],
        () => {
    ...
    it("should correctly handle current values of new sub sources", () => {
      ...
      const b = send(prop(), [value(1)]);
      const c = send(prop(), [value(2)]);
      expect(a.flatMapLatest()).to.emit([value(1), value(2)], () =>
        send(a, [value(b), value(c)])
      );
    });
    ...
    it("should work nicely with Kefir.constant and Kefir.never", () => {
      ...
      ).to.emit([value(3), value(4), value(5)], () =>
        send(a, [value(1), value(2), value(3), value(4), value(5)])
      );
    });
  });
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).flatMapLatest()).to.emit([
        end({ current: true })
      ]));
    ...
    it("should be ended if source was ended (with value)", () =>
      expect(
        send(prop(), [
          value(send(prop(), [value(0), end()])),
          end()
        ]).flatMapLatest()
      ).to.emit([value(0, { current: true }), end({ current: true })]));
