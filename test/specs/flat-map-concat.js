...
describe("flatMapConcat", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).flatMapConcat()).to.emit([
        end({ current: true })
      ]));
    ...
    it("should handle events", () => {
      ...
      const b = stream();
      const c = stream();
      expect(a.flatMapConcat()).to.emit(
        [value(1), value(2), value(5), value(6), end()],
        () => {
          send(b, [value(0)]);
          send(a, [value(b)]);
    ...
    it("should accept optional map fn", () => {
      const a = stream();
      const b = stream();
      expect(a.flatMapConcat(x => x.obs)).to.emit(
        [value(1), value(2), end()],
        () => {
          send(b, [value(0)]);
    ...
    it("should correctly handle current values of new sub sources", () => {
      ...
      expect(a.flatMapConcat()).to.emit([value(1), value(2)], () =>
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).flatMapConcat()).to.emit([
        end({ current: true })
      ]));
