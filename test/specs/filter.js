...
describe("filter", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(stream(), [end()]).filter(() => {})).to.emit([
        end({ current: true })
      ]);
    });
    ...
    it("should handle events", () => {
      const a = stream();
      expect(a.filter(x => x > 3)).to.emit(
        [value(4), value(5), error(7), value(6), end()],
        () =>
          send(a, [
            value(1),
            value(2),
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(prop(), [end()]).filter(() => {})).to.emit([
        end({ current: true })
      ]);
    });
    ...
    it("should handle events and current", () => {
      ...
      expect(a.filter(x => x > 2)).to.emit(
        [value(5, { current: true }), value(4), error(7), value(3), end()],
        () => send(a, [value(4), error(7), value(3), value(2), value(1), end()])
      );
      a = send(prop(), [error(0)]);
      expect(a.filter(x => x > 2)).to.emit(
        [error(0, { current: true }), value(4), error(7), value(3), end()],
        () => send(a, [value(4), error(7), value(3), value(2), value(1), end()])
      );
    });
    ...
    it("shoud use id as default predicate", () => {
      ...
      expect(a.filter()).to.emit(
        [value(4), error(-value(2)), value(5), value(6), end()],
        () =>
          send(a, [
            value(0),
            value(4),
