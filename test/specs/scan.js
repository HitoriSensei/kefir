...
describe("scan", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).scan(noop, 0)).to.emit([
    ...
    it("should handle events", () => {
      const a = stream();
      expect(a.scan(minus, 0)).to.emit(
        [value(0, { current: true }), value(-1), value(-4), end()],
    ...
    it("if no seed provided uses first value as seed", () => {
      const a = stream();
      expect(a.scan(minus)).to.emit(
        [value(0), value(-1), value(-4), end()],
    ...
    it("should never pass a value as current result if seed specified (test with error)", () => {
      ...
      deactivate(b);
      expect(
        handler.args.filter(xs => typeof xs[value(0)] === "number")
      ).to.deep.equal([]);
    });
    ...
    it("should use next value after error as seed, if seed not specified", () => {
      const a = stream();
      expect(a.scan(() => "abc")).to.emit(
        [value(1), value("abc"), error("err"), value(3), value("abc"), end()],
        () =>
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).scan(noop, 0)).to.emit([
        value(0, { current: true }),
        end({ current: true })
      ]));
    ...
    it("should handle events and current", () => {
      ...
      expect(a.scan(minus, 0)).to.emit(
        [value(-1, { current: true }), value(-4), value(-10), end()],
