...
describe("combine", () => {
  describe("array", () => {
    ...
    it("should accept optional combinator function", () => {
      ...
      a = stream();
      b = send(prop(), [value(0)]);
      expect(a.combine(b, join)).to.emit(
        [value("1+0"), value("1+2"), value("1+3"), end()],
        () => {
          send(a, [value(1)]);
          send(b, [value(2)]);
    ...
    describe("sampledBy al =>ity (3 arity combine)", () => {
      it("should stream", () => {
        expect(Kefir.combine([], [])).to.be.observable.stream();
        expect(
          Kefir.combine([stream(), prop()], [stream(), prop()])
      ...
      it("should be ended if array of ended observables provided", () => {
        ...
        const b = send(prop(), [end()]);
        const c = send(stream(), [end()]);
        expect(Kefir.combine([a, b, c], [stream(), prop()])).to.emit([
          end({ current: true })
        ]);
      });
      ...
      it("should work nice for emitating atomic updates with active props", () => {
        ...
        expect(combined.combine(c)).to.emit(
          [value([[3, 2], 2]), value([[4, 4], 4]), value([[5, 6], 6])],
          () => send(a, [value(1), value(2), value(3)])
        );
  ...
  describe("object", () => {
    ...
    describe("sampledBy al =>ity (3 arity combine)", () => {
      ...
      it("should be ended if empty array provided", () => {
        ...
        expect(Kefir.combine({}, { s2: stream(), p2: prop() })).to.emit([
          end({ current: true })
        ]);
      });
      ...
      it("should be ended if array of ended observables provided", () => {
        ...
        const b = send(prop(), [end()]);
        const c = send(stream(), [end()]);
        expect(Kefir.combine({ a, b, c }, { d: stream(), e: prop() })).to.emit([
          end({ current: true })
        ]);
      });
      ...
      it("should emit combined value synchronously", () => {
        ...
        const innerFooCombined = Kefir.combine([innerFooStream], x => x); // Some Kefir observable based on innerFooStream
