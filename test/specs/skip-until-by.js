...
describe("skipUntilBy", () => {
  ...
  describe("stream, stream", () => {
    ...
    it("should be ended if primary was ended", () =>
      expect(send(stream(), [end()]).skipUntilBy(stream())).to.emit([
        end({ current: true })
      ]));

    it("should be ended if secondary was ended", () =>
      expect(stream().skipUntilBy(send(stream(), [end()]))).to.emit([
        end({ current: true })
      ]));
  ...
  describe("stream, property", () => {
    ...
    it("should be ended if primary was ended", () =>
      expect(send(stream(), [end()]).skipUntilBy(prop())).to.emit([
        end({ current: true })
      ]));

    it("should be ended if secondary was ended and has no current", () =>
      expect(stream().skipUntilBy(send(prop(), [end()]))).to.emit([
        end({ current: true })
      ]));
  ...
  describe("property, stream", () => {
    ...
    it("should be ended if primary was ended", () =>
      expect(send(prop(), [end()]).skipUntilBy(stream())).to.emit([
        end({ current: true })
      ]));

    it("should be ended if secondary was ended", () =>
      expect(prop().skipUntilBy(send(stream(), [end()]))).to.emit([
        end({ current: true })
      ]));
  ...
  describe("property, property", () => {
    it("should return a property", () =>
      expect(prop().skipUntilBy(prop())).to.be.observable.property());
    ...
    it("should be ended if primary was ended", () =>
      expect(send(prop(), [end()]).skipUntilBy(prop())).to.emit([
        end({ current: true })
      ]));

    it("should be ended if secondary was ended and has no current", () =>
      expect(prop().skipUntilBy(send(prop(), [end()]))).to.emit([
        end({ current: true })
      ]));
