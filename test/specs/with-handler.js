...
describe("withHandler", () => {
  ...
  describe("stream", () => {
    ...
    it("should be ended if source was ended (with `mirror` handler)", () =>
      expect(send(stream(), [end()]).withHandler(mirror)).to.emit([
        end({ current: true })
      ]));
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended (with `mirror` handler)", () =>
      expect(send(prop(), [end()]).withHandler(mirror)).to.emit([
        end({ current: true })
      ]));
    ...
    it("should support emitter.emitEvent", () => {
      ...
      expect(
        send(prop(), [error(-1, { current: true })]).withHandler(
          emitEventMirror
        )
      ).to.emit([error(-1, { current: true })]);
    });
    ...
    it("should automatically preserve isCurent (value)", () => {
      const a = prop();
      expect(a.withHandler(mirror)).to.emit([value(1)], () =>
        send(a, [value(1)])
      );
      ...
      ).to.emit([value(1, { current: true }), value(2)], () =>
        savedEmitter.value(2)
      );
    });

    it("should automatically preserve isCurent (error)", () => {
      const a = prop();
      expect(a.withHandler(mirror)).to.emit([error(1)], () =>
        send(a, [error(1)])
      );
      ...
      ).to.emit([error(1, { current: true }), error(2)], () =>
        savedEmitter.error(2)
      );
    });
  });
});
