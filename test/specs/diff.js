...
describe("diff", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(stream(), [end()]).diff(noop, 0)).to.emit([
        end({ current: true })
      ]);
    });
    ...
    it("should handle events", () => {
      const a = stream();
      expect(a.diff(minus, 0)).to.emit([value(-1), value(-2), end()], () =>
    ...
    it("works without fn argument", () => {
      const a = stream();
      expect(a.diff(null, 0)).to.emit(
        [value([0, 1]), value([1, 3]), end()],
        () => send(a, [value(1), value(3), end()])
      );
    });
    ...
    it("if no seed provided uses first value as seed", () => {
      let a = stream();
      expect(a.diff(minus)).to.emit([value(-1), value(-2), end()], () =>
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(prop(), [end()]).diff(noop, 0)).to.emit([
        end({ current: true })
      ]);
    });
    ...
    it("works without fn argument", () => {
      const a = send(prop(), [value(1)]);
      expect(a.diff(null, 0)).to.emit(
        [value([0, 1], { current: true }), value([1, 3]), value([3, 6]), end()],
        () => send(a, [value(3), value(6), end()])
      );
    });
