...
describe("slidingWindow", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).slidingWindow(1)).to.emit([
        end({ current: true })
      ]));
    ...
    it(".slidingWindow(3, 3) should work correctly", () => {
      const a = stream();
      expect(a.slidingWindow(3, 3)).to.emit(
        [value([1, 2, 3]), value([2, 3, 4]), value([3, 4, 5]), end()],
        () => send(a, [value(1), value(2), value(3), value(4), value(5), end()])
      );
    });
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).slidingWindow(1)).to.emit([
        end({ current: true })
      ]));
