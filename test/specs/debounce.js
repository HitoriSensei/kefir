...
describe("debounce", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(stream(), [end()]).debounce(100)).to.emit([
        end({ current: true })
      ]);
    });
    ...
    it("should handle events", () => {
      ...
      expect(a.debounce(100)).to.emitInTime(
        [[160, value(3)], [360, value(4)], [710, value(8)], [710, end()]],
        tick => {
          send(a, [value(1)]);
          tick(30);
          send(a, [value(2)]);
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(prop(), [end()]).debounce(100)).to.emit([
        end({ current: true })
      ]);
    });

    it("should be ended if source was ended (with current)", () => {
      expect(send(prop(), [value(1), end()]).debounce(100)).to.emit([
    ...
    it("should end immediately if no value to emit later", () => {
      const a = send(prop(), [value(0)]);
      expect(a.debounce(100)).to.emitInTime(
        [[0, value(0, { current: true })], [100, value(1)], [200, end()]],
        tick => {
          send(a, [value(1)]);
          tick(200);
          send(a, [end()]);
