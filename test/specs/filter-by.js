...
describe("filterBy", () => {
  ...
  describe("stream, stream", () => {
    ...
    it("should be ended if primary was ended", () => {
      expect(send(stream(), [end()]).filterBy(stream())).to.emit([
        end({ current: true })
      ]);
    });

    it("should be ended if secondary was ended", () => {
      expect(stream().filterBy(send(stream(), [end()]))).to.emit([
        end({ current: true })
      ]);
    });
    ...
    it("should end when secondary ends if last value from it was falsey", () => {
      const a = stream();
      const b = stream();
      expect(a.filterBy(b)).to.emit([end()], () =>
    ...
    it("should filter values as expected", () => {
      const a = stream();
      const b = stream();
      expect(a.filterBy(b)).to.emit(
        [value(3), value(4), value(7), value(8), end()],
        () => {
          send(b, [value(true)]);
  ...
  describe("stream, property", () => {
    ...
    it("should be ended if primary was ended", () => {
      expect(send(stream(), [end()]).filterBy(prop())).to.emit([
        end({ current: true })
      ]);
    });

    it("should be ended if secondary was ended and has no current", () => {
      expect(stream().filterBy(send(prop(), [end()]))).to.emit([
        end({ current: true })
      ]);
    });

    it("should be ended if secondary was ended and has falsey current", () => {
      expect(stream().filterBy(send(prop(), [value(false), end()]))).to.emit([
        end({ current: true })
      ]);
    });
  ...
  describe("property, stream", () => {
    ...
    it("should be ended if primary was ended", () => {
      expect(send(prop(), [end()]).filterBy(stream())).to.emit([
        end({ current: true })
      ]);
    });

    it("should be ended if secondary was ended", () => {
      expect(prop().filterBy(send(stream(), [end()]))).to.emit([
        end({ current: true })
      ]);
    });
  ...
  describe("property, property", () => {
    ...
    it("should be ended if primary was ended", () => {
      expect(send(prop(), [end()]).filterBy(prop())).to.emit([
        end({ current: true })
      ]);
    });

    it("should be ended if secondary was ended and has no current", () => {
      expect(prop().filterBy(send(prop(), [end()]))).to.emit([
        end({ current: true })
      ]);
    });

    it("should be ended if secondary was ended and has falsey current", () => {
      expect(prop().filterBy(send(prop(), [value(false), end()]))).to.emit([
        end({ current: true })
      ]);
    });
