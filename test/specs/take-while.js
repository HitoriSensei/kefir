...
describe("takeWhile", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(stream(), [end()]).takeWhile(() => true)).to.emit([
        end({ current: true })
      ]);
    });
    ...
    it("should handle events", () => {
      const a = stream();
      expect(a.takeWhile(x => x < 4)).to.emit(
    ...
    it("should handle events (natural end)", () => {
      const a = stream();
      expect(a.takeWhile(x => x < 4)).to.emit([value(1), value(2), end()], () =>
    ...
    it("should handle events (with `-> false`)", () => {
      const a = stream();
      expect(a.takeWhile(() => false)).to.emit([end()], () =>
        send(a, [value(1), value(2), end()])
      );
    });
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).takeWhile(() => true)).to.emit([
        end({ current: true })
      ]));

    it("should be ended if calback was `-> false` and source has a current", () =>
      expect(send(prop(), [value(1)]).takeWhile(() => false)).to.emit([
        end({ current: true })
      ]));
    ...
    it("should handle events", () => {
      ...
      expect(a.takeWhile(x => x < 4)).to.emit(
        [value(1, { current: true }), value(2), value(3), end()],
    ...
    it("shoud use id as default predicate", () => {
      ...
      expect(a.takeWhile()).to.emit(
        [value(1, { current: true }), value(2), end()],
        () => send(a, [value(2), value(0), value(5), end()])
      );
      a = send(prop(), [value(0)]);
      expect(a.takeWhile()).to.emit([end({ current: true })], () =>
        send(a, [value(2), value(0), value(5), end()])
      );
    });
