...
describe("bufferWhileBy", () => {
  describe("common", () => {
    ...
    it("should flush empty buffer and then end when primary ends", () => {
      ...
      const a = stream();
      const b = stream();
      expect(a.bufferWhileBy(b)).to.emit([value([]), end()], () =>
        send(a, [end()])
      );
    });
    ...
    it("should flush empty buffer when secondary emits false (w/ {flushOnChange: true})", () => {
      expect(
        stream().bufferWhileBy(send(prop(), [value(false)]), {
          flushOnChange: true
        })
      ).to.emit([value([], { current: true })]);
      const a = stream();
      const b = stream();
      expect(a.bufferWhileBy(b, { flushOnChange: true })).to.emit(
        [value([])],
        () => send(b, [value(true), value(false)])
      );
    });
    ...
    it("should flush buffer on end", () => {
      ...
      const a = stream();
      const b = stream();
      expect(a.bufferWhileBy(b)).to.emit([value([1, 2]), end()], () =>
    ...
    it("should not flush buffer on end if {flushOnEnd: false}", () => {
      expect(
        send(prop(), [value(1), end()]).bufferWhileBy(stream(), {
          flushOnEnd: false
        })
      ).to.emit([end({ current: true })]);
      const a = stream();
      const b = stream();
      expect(a.bufferWhileBy(b, { flushOnEnd: false })).to.emit([end()], () =>
        send(a, [value(1), value(2), end()])
      );
    });
    ...
    it("should end when secondary ends, if it haven't emitted any value (w/ {flushOnEnd: false})", () => {
      expect(
        stream().bufferWhileBy(send(stream(), [end()]), { flushOnEnd: false })
      ).to.emit([end({ current: true })]);
      const a = stream();
      const b = stream();
      expect(a.bufferWhileBy(b, { flushOnEnd: false })).to.emit([end()], () =>
    ...
    it("should end when secondary ends, if its last emitted value was truthy (w/ {flushOnEnd: false})", () => {
      expect(
        stream().bufferWhileBy(send(prop(), [value(true), end()]), {
          flushOnEnd: false
        })
      ).to.emit([end({ current: true })]);
      const a = stream();
      const b = stream();
      expect(a.bufferWhileBy(b, { flushOnEnd: false })).to.emit([end()], () =>
        send(b, [value(true), end()])
      );
    });

    it("should not end when secondary ends, if its last emitted value was falsy (w/ {flushOnEnd: false})", () => {
      expect(
        stream().bufferWhileBy(send(prop(), [value(false), end()]), {
          flushOnEnd: false
        })
      ).to.emit([]);
      const a = stream();
      const b = stream();
      expect(a.bufferWhileBy(b, { flushOnEnd: false })).to.emit([], () =>
        send(b, [value(false), end()])
      );
    });
    ...
    it("should flush buffer on each value from primary if last value form secondary was falsy", () => {
      ...
      const b = stream();
      expect(a.bufferWhileBy(b)).to.emit(
        [value([1, 2, 3, 4]), value([5]), value([6, 7, 8])],
        () => {
