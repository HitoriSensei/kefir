...
describe("flatMapFirst", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).flatMapFirst()).to.emit([
        end({ current: true })
      ]));
    ...
    it("should handle events", () => {
      ...
      const b = stream();
      const c = stream();
      expect(a.flatMapFirst()).to.emit(
        [value(1), value(2), value(4), end()],
        () => {
          send(b, [value(0)]);
          send(a, [value(b)]);
          send(b, [value(1)]);
    ...
    it("should accept optional map fn", () => {
      const a = stream();
      const b = stream();
      expect(a.flatMapFirst(x => x.obs)).to.emit(
        [value(1), value(2), end()],
        () => {
    ...
    it("should correctly handle current values of new sub sources", () => {
      ...
      expect(a.flatMapFirst()).to.emit([value(1), value(2)], () =>
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).flatMapFirst()).to.emit([
        end({ current: true })
      ]));
