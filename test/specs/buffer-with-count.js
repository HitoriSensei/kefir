...
describe("bufferWithCount", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(stream(), [end()]).bufferWithCount(1)).to.emit([
        end({ current: true })
      ]);
    });
    ...
    it(".bufferWithCount(1) should work correctly", () => {
      ...
      expect(a.bufferWithCount(1)).to.emit(
        [value([1]), value([2]), value([3]), value([4]), value([5]), end()],
    ...
    it(".bufferWithCount(2) should work correctly", () => {
      const a = stream();
      expect(a.bufferWithCount(2)).to.emit(
        [value([1, 2]), value([3, 4]), value([5]), end()],
        () => send(a, [value(1), value(2), value(3), value(4), value(5), end()])
      );
    });
    ...
    it(".bufferWithCount(3) should work correctly", () => {
      const a = stream();
      expect(a.bufferWithCount(3)).to.emit(
        [value([1, 2, 3]), value([4, 5]), end()],
        () => send(a, [value(1), value(2), value(3), value(4), value(5), end()])
      );
    });
    ...
    it("should not flush buffer on end if {flushOnEnd: false}", () => {
      const a = stream();
      expect(a.bufferWithCount(3, { flushOnEnd: false })).to.emit(
        [value([1, 2, 3]), end()],
        () => send(a, [value(1), value(2), value(3), value(4), value(5), end()])
      );
    });
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(prop(), [end()]).bufferWithCount(1)).to.emit([
        end({ current: true })
      ]);
    });
