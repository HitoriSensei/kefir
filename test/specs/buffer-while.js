...
describe("bufferWhile", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).bufferWhile(not3)).to.emit([
        end({ current: true })
      ]));
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(prop(), [end()]).bufferWhile(not3)).to.emit([
        end({ current: true })
      ]);
      ...
      expect(
        send(prop(), [value(3), end()]).bufferWhile(not3, { flushOnEnd: false })
      ).to.emit([value([3], { current: true }), end({ current: true })]);
      expect(
        send(prop(), [value(2), end()]).bufferWhile(not3, { flushOnEnd: false })
