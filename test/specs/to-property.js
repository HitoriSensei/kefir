...
describe("toProperty", () => {
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).toProperty()).to.emit([
        end({ current: true })
      ]));

    it("should be ended if source was ended (with current)", () =>
      expect(send(stream(), [end()]).toProperty(() => 0)).to.emit([
    ...
    it("should handle events", () => {
      ...
      expect(p).to.emit(
        [value(0, { current: true }), value(1), error(3), value(2), end()],
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () => {
      expect(send(prop(), [end()]).toProperty(() => 0)).to.emit([
        value(0, { current: true }),
        end({ current: true })
      ]);
    ...
    it("should handle events", () => {
      ...
      expect(b).to.emit(
        [value(1, { current: true }), value(2), error(3), end()],
      ...
      b = a.toProperty(() => 0);
      expect(b).to.emit(
        [value(0, { current: true }), value(2), error(3), value(4), end()],
