...
const testWithLib = (name, t) =>
  describe(`with ${name} implementation`, () => {
    describe("stream", () => {
      it("`map` should work", () => {
        const a = stream();
        expect(a.transduce(t.map(x => x * 2))).to.emit(
          [value(2), value(4), value(6), end()],
      ...
      it("`filter` should work", () => {
        const a = stream();
        expect(a.transduce(t.filter(x => x % 2 === 0))).to.emit(
          [value(2), value(4), end()],
      ...
      it("`take` should work", () => {
        let a = stream();
        expect(a.transduce(t.take(2))).to.emit(
          [value(1), value(2), end()],
          () => send(a, [value(1), value(2), value(3), value(4)])
        );
        a = stream();
        expect(a.transduce(t.take(2))).to.emit([value(1), end()], () =>
          send(a, [value(1), end()])
        );
      });
      ...
      if (t.partitionAll) {
        it("`partitionAll` should work", () => {
          let a = stream();
          expect(a.transduce(t.partitionAll(2))).to.emit(
            [value([1, 2]), value([3, 4]), end()],
            () => send(a, [value(1), value(2), value(3), value(4), end()])
          );
          a = stream();
          expect(a.transduce(t.partitionAll(2))).to.emit(
            [value([1, 2]), value([3]), end()],
            () => send(a, [value(1), value(2), value(3), end()])
          );
        });
    ...
    describe("property", () => {
      it("`map` should work", () => {
        ...
        expect(a.transduce(t.map(x => x * 2))).to.emit(
          [value(2, { current: true }), value(4), value(6), end()],
      ...
      it("`filter` should work", () => {
        ...
        a = send(prop(), [value(2)]);
        expect(a.transduce(t.filter(x => x % 2 === 0))).to.emit(
          [value(2, { current: true }), value(4), end()],
      ...
      it("`take` should work", () => {
        let a = send(prop(), [value(1)]);
        expect(a.transduce(t.take(2))).to.emit(
          [value(1, { current: true }), value(2), end()],
          () => send(a, [value(2), value(3), value(4)])
        );
        a = send(prop(), [value(1)]);
        expect(a.transduce(t.take(3))).to.emit(
          [value(1, { current: true }), value(2), end()],
          () => send(a, [value(2), end()])
        );
      });
      ...
      it("`map.filter.take` should work", () => {
        ...
        a = send(prop(), [value(2)]);
        expect(a.transduce(tr)).to.emit(
          [value(12, { current: true }), value(14), end()],
  ...
  describe("with `noop` transducer", () => {
    describe("stream", () => {
      ...
      it("should be ended if source was ended", () =>
        expect(send(stream(), [end()]).transduce(noop)).to.emit([
          end({ current: true })
        ]));

      it("should handle events", () => {
        const a = stream();
        expect(a.transduce(noop)).to.emit(
          [value(1), value(2), error(4), value(3), end()],
    ...
    describe("property", () => {
      ...
      it("should be ended if source was ended", () =>
        expect(send(prop(), [end()]).transduce(noop)).to.emit([
          end({ current: true })
        ]));
      ...
      it("should handle events and current", () => {
        ...
        a = send(prop(), [error(0)]);
        expect(a.transduce(noop)).to.emit(
          [error(0, { current: true }), value(2), error(4), value(3), end()],
          () => send(a, [value(2), error(4), value(3), end()])
        );
      });
    });
