...
describe("skipDuplicates", () => {
  ...
  describe("stream", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(stream(), [end()]).skipDuplicates()).to.emit([
        end({ current: true })
      ]));

    it("should handle events (default comparator)", () => {
      const a = stream();
      expect(a.skipDuplicates()).to.emit(
    ...
    it("should handle events (custom comparator)", () => {
      const a = stream();
      expect(a.skipDuplicates(roundlyEqual)).to.emit(
        [value(1), value(2), value(3.8), end()],
        () =>
    ...
    it("should help with creating circular dependencies", () => {
      // https://github.com/kefirjs/kefir/issues/42
  ...
  describe("property", () => {
    ...
    it("should be ended if source was ended", () =>
      expect(send(prop(), [end()]).skipDuplicates()).to.emit([
        end({ current: true })
      ]));
